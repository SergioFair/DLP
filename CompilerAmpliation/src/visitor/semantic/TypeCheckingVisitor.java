...
public class TypeCheckingVisitor extends AbstractVisitor {
	...
	public Object visit(Comparison compar, Object params) {
		...
		compar.setType(compar.getLeft().getType()
				.comparison(compar.getRight().getType()));
	...
	public Object visit(Indexing ind, Object params) {
		...
		ind.setLValue(true);
		ind.setType(ind.getLeft().getType()
				.squareBrackets(ind.getRight().getType()));
		if (ind.getType() == null)
	...
	public Object visit(Logical log, Object params) {
		...
		log.setType(log.getLeft().getType()
				.comparison(log.getRight().getType()));
		if (log.getType() == null)
		...
		if (as.getLeft().getType() instanceof ArrayType)
			as.getLeft().setType(
					as.getRight()
							.getType()
							.promotesTo(
									((ArrayType) as.getLeft().getType())
											.getOf()));
		else
			as.getLeft().setType(
					as.getRight().getType().promotesTo(as.getLeft().getType()));
	...
	public Object visit(ForStatement forStatement, Object params) {
		if (!(forStatement.getInitilization() instanceof Assignment))
			new ErrorType(forStatement.getInitilization(),
					"Initialization in For Statement is not correct");
		...
		if (!(exp instanceof Increment || exp instanceof ArithmeticAssignment || exp instanceof Decrement))
			new ErrorType(forStatement.getIncrement(),
					"Increment in For Statement is not correct");
	...
	public Object visit(Return ret, Object params) {
		...
		if (ret.getExpression().getType()
				.promotesTo(((FunctionType) params).getReturnType()) == null)
